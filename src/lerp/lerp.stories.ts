...
const meta = {
  ...
  render: ({ rate }) => {
    ...
    title.textContent = "Lerp — Mouse Follow";
    title.style.cssText = "margin:0;color:#ccc;font-weight:400;font-size:18px;";
    container.appendChild(title);

    const description = document.createElement("p");
    description.textContent =
      "Move your mouse over the track — the block exponentially approaches it";
    ...
    velocityFill.style.cssText =
      "position:absolute;top:0;left:50%;height:100%;width:0%;background:#e5c07b;border-radius:3px;";
    velocityBar.appendChild(velocityFill);

    const velocityValue = document.createElement("span");
    velocityValue.textContent = "0.00";
    velocityValue.style.cssText = "min-width:60px;text-align:right;color:#e5c07b;";
    ...
    const lerp = createLerp({
      ...
      onUpdate: (value, velocity) => {
        ...
        velocityFill.style.background = velocity >= 0 ? "#e5c07b" : "#e06c75";
