...
## [0.3.0] - Pre-release — API subject to change before 1.0.0.
...
- `createTimeline` — orchestrates multiple keyframe animations on a shared timeline with `at`/`gap` positioning. Takes `KeyframedAnimationOptions` configs directly. Registers a single ticker handler regardless of layer count.
